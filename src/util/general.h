...
void invoke_func_nolock(const std::function<void(args_...)>& ref, args_... args) {
    if (ref != nullptr) {
        try {
            ref(args...);
...
void invoke_sharedptr_nolock(const std::shared_ptr<obj_>& ref, void(obj_::*func)(args_...), args_... args) {
    auto ptr = ref.get();
    if (ptr != nullptr) {
        try {
            (ptr->*func)(args...);
